...
public:
    ...
    enum
    {
        ...
        //! When the mouse is clicked the SCN_MARGINCLICK() signal is Q_EMITted.
        //!
    ...
    enum
    {
        //! No lexer is selected and the SCN_STYLENEEDED signal is Q_EMITted so
...
Q_SIGNALS:
    //! This signal is Q_EMITted when text is selected or de-selected.
    //! \a yes is true if text has been selected and false if text has been
    //! deselected.
    void QSCN_SELCHANGED(bool yes);

    //! This signal is Q_EMITted when the user cancels an auto-completion list.
    //!
    //! \sa SCN_AUTOCSELECTION()
    void SCN_AUTOCCANCELLED();

    //! This signal is Q_EMITted when the user deletes a character when an
    //! auto-completion list is active.
    void SCN_AUTOCCHARDELETED();

    //! This signal is Q_EMITted when the user selects an item in an
    //! auto-completion list.  It is Q_EMITted before the selection is inserted.
    ...
    //! This signal is Q_EMITted when the document has changed for any reason.
    void SCEN_CHANGE();

    //! This signal ir Q_EMITted when the user clicks on a calltip.
    ...
    //! This signal is Q_EMITted whenever the user enters an ordinary character
    //! into the text.
    ...
    //! This signal is Q_EMITted when the user double clicks.
    ...
    //! This signal is Q_EMITted when the user clicks on text in a style with the
    ...
    //! This signal is Q_EMITted when the user double clicks on text in a style
    ...
    //! This signal is Q_EMITted when the user releases the mouse button on text
    ...
    //! This signal is Q_EMITted when the user clicks on text that has an
    //! indicator.
    //! \a position is the position in the text where the click occured.
    //! \a modifiers is the logical or of the modifier keys that were pressed
    //! when the user clicked.
    ...
    //! This signal is Q_EMITted when a recordable editor command has been
    //! executed.
    void SCN_MACRORECORD(unsigned int, unsigned long, void *);

    //! This signal is Q_EMITted when the user clicks on a sensitive margin.
    ...
    //! This signal is Q_EMITted when the user attempts to modify read-only
    //! text.
    ...
    //! This signal is Q_EMITted when a range of text needs to be syntax styled.
    //! The range is from the value returned by the SCI_GETENDSTYLED message
    //! and \a position.  It is only Q_EMITted if the currently selected lexer is
    //! SCLEX_CONTAINER.
    //!
    //! \sa SCI_COLOURISE, SCI_GETENDSTYLED
    void SCN_STYLENEEDED(int position);

    //! This signal is Q_EMITted when either the text or styling of the text has
    ...
    //! since the signal was last Q_EMITted.
...
private:
    // This is needed to allow QsciScintillaQt to Q_EMIT this class's Q_SIGNALS.
